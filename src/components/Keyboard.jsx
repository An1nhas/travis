...
import { MdBackspace, MdSpaceBar, MdKeyboardReturn } from "react-icons/md";
...
const Keyboard = ({ keyboard, keyboardSet, shift, keyLang, handleClick,
    ...
    const keyLayout = keyboard.length ? (
        keyboard.map(key => {
            ...
            const handleClickAll = keyboardSet === 0 ? handleClickTgr : handleClick;
            const keyCurrent = (keyboardSet === 0 && shift === false) ? key.keyTgr :
                (keyboardSet === 0 && shift === true) ? key.keyTgrShift :
                    keyboardSet === 1 ? key.keyNum : key.keyPunct;
            const keyPrint = key.id === 27 ? <MdBackspace /> :
